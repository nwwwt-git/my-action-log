...
toggleAdminBtn.onclick = () => {
  ...
  toggleAdminBtn.innerHTML = isHidden ? '<i class="fa-solid fa-gear"></i> 管理を表示' : '<i class="fa-solid fa-xmark"></i> 閉じる';
};
...
function render() {
  ...
  Object.keys(groups).sort().reverse().forEach(month => {
    ...
    monthGroup.innerHTML = `<div class="month-header" onclick="this.parentElement.classList.toggle('open')"><span>${month.replace('-', '年')}月</span><i class="fa-solid fa-chevron-down" style="margin-left:auto; font-size:0.8rem; color:#94a3b8;"></i></div><div class="month-content"><ul style="padding:0; margin:0;"></ul></div>`;
    ...
    groups[month].forEach((record, i) => {
      ...
      if (datePart !== lastDate) {
        ...
        dayHeader.textContent = `${d.getMonth()+1}月${d.getDate()}日 (${["日","月","火","水","木","金","土"][d.getDay()]})`;
      ...
      li.querySelector('.memo-text').onclick = () => { const m = prompt("メモ:", record.memo); if(m!==null){ records[recordIndex].memo = m; saveAndRender(); }};
...
importFile.onchange = (e) => {
  ...
  reader.onload = (ev) => {
    try {
      ...
      if(Array.isArray(data) && confirm("データを復元しますか？")) {
...
deleteMonthBtn.onclick = () => {
  const m = targetMonthInput.value;
  if(m && confirm(`${m}の全データを消しますか？`)) { records = records.filter(r => !r.time.startsWith(m)); saveAndRender(); }
};
